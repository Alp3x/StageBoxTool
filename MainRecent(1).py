...
SYNC_POLL_MS      = 200        # local diff/push tick — kept short so edits feel live
SYNC_RECONNECT_MS = 2500       # delay between client auto-reconnect attempts
SYNC_HEARTBEAT_S  = 4.0        # idle time before a keepalive ping is sent
...
class SyncManager:
    def __init__(self, app):
        ...
        self._poll_job = None
        # Auto-reconnect / liveness tracking (client side). `_want_client`
        # stays True while the user intends to be connected, independent of
        # the current live socket state, so a dropped connection can be
        # re-established transparently instead of just going silent.
        self._want_client = False
    ...
    def _configure_keepalive(sock):
        """Enable OS-level TCP keepalive so a dropped link (cable pulled,
        wifi lost, app killed) is detected within a few seconds instead of
        the socket hanging silently until the next write fails."""
        try:
    ...
    def start_client(self, host_ip, port, _is_reconnect=False):
        if not _is_reconnect:
            self.stop()
            self._want_client = True
        self._stop_flag.clear()
        ...
        try:
            ...
            self.role_status = f"Reconnecting… ({e})" if self._want_client else f"Fail: {e}"
            if self._want_client:
        ...
        self.last_sent_hash = None       # force a full resync handshake
        threading.Thread(target=self._recv_loop, args=(sock,), daemon=True).start()
        self._start_poll()
        return True

    def _schedule_reconnect(self):
        if self._reconnect_job or not self._want_client:
            return
        self._reconnect_job = self.app.after(SYNC_RECONNECT_MS, self._attempt_reconnect)

    def _attempt_reconnect(self):
        self._reconnect_job = None
        if not self._want_client:
            return
        self.start_client(self.host_ip, self.port, _is_reconnect=True)
    ...
    def stop(self):
        ...
        if self._reconnect_job:
            try: self.app.after_cancel(self._reconnect_job)
    ...
    def _recv_loop(self, sock):
        try:
            ...
            if self.mode == "host":
                ...
                self.client_sock = None
                if self._want_client and not self._stop_flag.is_set():
                    self.role_status = "Disconnected — reconnecting…"
...
class App(tk.Tk):
    ...
    def _apply_theme_specs(self):
        ...
        # NOTE: the clam theme draws its combobox field border using
        # "lightcolor"/"darkcolor" (bevel highlight/shadow) in addition to
        # "bordercolor". If those aren't explicitly matched to the theme,
        # clam falls back to its default light bevel, which shows up as a
        # thin white line around the dropdown. Pin every border-related
        # color (including per-state variants) to the theme palette so the
        # box reads as a flat, borderless field.
        s.configure("TCombobox",
                     selectbackground=BG2, selectforeground=TEXT,
                     background=BG3, foreground=TEXT, arrowcolor=TEXT_DIM,
                     fieldbackground=BG3, bordercolor=BORDER,
                     lightcolor=BG3, darkcolor=BG3,
                     borderwidth=1, relief="flat", padding=4)
        s.map("TCombobox",
              fieldbackground=[("readonly", BG3), ("focus", BG3), ("disabled", BG2)],
              background=[("readonly", BG3), ("focus", BG3)],
              foreground=[("readonly", TEXT)],
              bordercolor=[("readonly", BORDER), ("focus", ACCENT), ("!focus", BORDER)],
              lightcolor=[("readonly", BG3), ("focus", BG3), ("!focus", BG3)],
              darkcolor=[("readonly", BG3), ("focus", BG3), ("!focus", BG3)],
              arrowcolor=[("active", TEXT), ("pressed", ACCENT2)])

        # The popdown list is a plain Tk Listbox spawned outside the ttk
        # style system, so it needs its own option_add overrides or it
        # renders with Tk's default white highlight border around it.
        self.option_add("*TCombobox*Listbox.background", BG3)
        self.option_add("*TCombobox*Listbox.foreground", TEXT)
        self.option_add("*TCombobox*Listbox.selectBackground", ACCENT)
        self.option_add("*TCombobox*Listbox.selectForeground", DARK_TXT)
        self.option_add("*TCombobox*Listbox.borderWidth", 0)
        self.option_add("*TCombobox*Listbox.highlightThickness", 1)
        self.option_add("*TCombobox*Listbox.highlightBackground", BORDER)
        self.option_add("*TCombobox*Listbox.highlightColor", BORDER)
